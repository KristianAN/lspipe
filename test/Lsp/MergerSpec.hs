...
getMergedCapabilities :: T.Text -> Maybe Object
getMergedCapabilities txt = do
    Object obj <- decode (BL.fromStrict (TE.encodeUtf8 txt)) :: Maybe Value
    Object res <- parseMaybe (.: "result") obj
    Object caps <- parseMaybe (.: "capabilities") res
    return caps

allCapabilityKeys :: [T.Text]
allCapabilityKeys =
...
spec = do
    describe "Lsp.Merger.mergeCapapilities" $ do
        it "merges all capabilities from both servers" $ do
            let merged = mergeLspInitMessages ["hls", "typo"] [hlsResponse, typoResponse]
            merged
                `shouldSatisfy` ( \case
                                    Just txt ->
                                        case getMergedCapabilities txt of
                                            Just caps ->
                                                sort (KM.keys caps) == sort (Prelude.map fromText allCapabilityKeys)
                                            _ -> False
                                    _ -> False
                                )
        it "merges subfields for shared capabilities" $ do
            let merged = mergeLspInitMessages ["hls", "typo"] [hlsResponse, typoResponse]
            let mCodeActionKinds = do
                    txt <- merged
                    caps <- getMergedCapabilities txt
                    Object codeAction <- KM.lookup "codeActionProvider" caps
                    KM.lookup "codeActionKinds" codeAction
            mCodeActionKinds `shouldBe` Just (Array $ V.fromList [String "quickfix"])
